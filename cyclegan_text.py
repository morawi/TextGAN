...
def train_batch(i, epoch, batch, prev_time, real_A, real_B):
    ...
    return loss_id_A.cpu().data.detach().numpy().tolist(), loss_id_B.cpu().data.detach().numpy().tolist(), prev_time
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        # Set model input        
        loss_id_A, loss_id_B, prev_time = train_batch(i, epoch, batch, prev_time, 
                                               Variable(batch['A'].type(Tensor)), 
                                               Variable(batch['B'].type(Tensor)))  
